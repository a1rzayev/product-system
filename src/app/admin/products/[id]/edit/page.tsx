...
export default function EditProductPage({ params }: EditProductPageProps) {
  ...
  const setPrimaryImage = async (imageId: string) => {
    try {
      ...
      // Update all images to set the new primary
      ...
      const updatedImagesData = await response.json()
      setExistingImages(updatedImagesData)
    } catch (error) {
      ...
      <form onSubmit={handleSubmit} className="bg-white shadow rounded-lg p-6 space-y-6">
        ...
        <div>
          ...
          {existingImages.length > 0 && (
            <div className="mb-6">
              <h4 className="text-sm font-medium text-black mb-2">{t('products.currentImages')} ({existingImages.length})</h4>
                ...
                {existingImages.map((image, index) => (
                  <div key={image.id} className="relative group">
                    ...
                    <div className="absolute top-1 left-1">
                      {image.isPrimary && (
                        <span className="bg-blue-500 text-white text-xs px-1 py-0.5 rounded">{t('products.primary')}</span>
                      )}
                    </div>
                    <div className="absolute top-1 right-1 space-x-1">
                      {!image.isPrimary && (
                        <button
                          ...
                          className="bg-blue-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-xs hover:bg-blue-600 opacity-0 group-hover:opacity-100 transition-opacity disabled:opacity-50"
                      ...
                      <button
                        ...
                        className="bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-xs hover:bg-red-600 opacity-0 group-hover:opacity-100 transition-opacity"
          ...
          {newImages.length > 0 && (
            <div className="mt-4">
              <h4 className="text-sm font-medium text-black mb-2">{t('products.newImagesToUpload')} ({newImages.length})</h4>
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
